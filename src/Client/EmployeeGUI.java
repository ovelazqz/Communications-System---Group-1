...
public class EmployeeGUI implements UserInterface {
    ...
    private java.util.Map<String, java.util.List<String>> chatMessages = new java.util.HashMap<>();
    ...
    public void handleIncomingMessage(Message message) {
    	SwingUtilities.invokeLater(() -> {
            if (message.getType().equals("receive_message")) {
                ...
                if (sessionID.equals("get_user_list") && status.equals("success")) {
    ...
    private void selectChat(String chatName) {
        ...
        if (chatMessages.containsKey(chatName)) {
            for (String msg : chatMessages.get(chatName)) {
    ...
    private void createGroupChat() {
        ...
        if (groupName != null && !groupName.trim().isEmpty()) {
            ...
            int result = JOptionPane.showConfirmDialog(frame, scrollPane, 
                    "Select members to add:", JOptionPane.OK_CANCEL_OPTION);
            
            if (result == JOptionPane.OK_OPTION) {
                String[] selectedMembers = userList.getSelectedValuesList().toArray(new String[0]);
                
                if (selectedMembers.length == 0) {
                    JOptionPane.showMessageDialog(frame, "Please select at least one member");
                    return;
                }
                
                try {
                    client.sendCreateGroupChatRequest(groupName, selectedMembers);
    ...
    private void addMessage(String message, boolean isOther) {
        // Store the message
        if (selectedChat != null) {
            chatMessages.putIfAbsent(selectedChat, new java.util.ArrayList<>());
            chatMessages.get(selectedChat).add(message);
        }
